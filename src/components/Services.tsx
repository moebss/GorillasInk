      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {services.map((svc, idx) => (
            <motion.div key={svc.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.2 }} className={`p-8 md:p-12 border ${svc.highlight ? 'border-gold' : 'border-white/10'} bg-darker rounded-sm relative`}>
